...
def check_path(devicePath, relayCount=None):
    ...
    try:
        ...
        serialDevice = serial.Serial(
            devicePath, timeout=SERIAL_DEFAULT_TIMEOUT)
...
class relayModule():
            ...
            if self.type == SERIAL_TYPE:
                try:
                    if self.device.write(command) != 1:
                        log.error(
                            'Unexpected Error: Serial Write diff of 1 byte')
    ...
    def __set_relay_count__(self):
        ...
        # request states from device, read hex response and convert to bin
        # strng
    ...
    def set_relay_position(self, relay, position):
        ...
        if isinstance(relay, int) and relay in range(0, self.relayCount + 1):
            # position must be an integer 0-1
            if isinstance(position, int) and position in range(0, 2):
                # set relay position
                if self.__send_relay_command__(
                    ...
                    log.error(
                        'error in set_relay_position sending relay command')
            else:
                log.error('position must be 0 or 1')
        else:
            log.error(
                'relay must be 0 (all relays) or 1 - %s',
                self.relayCount)
        return False
    ...
    def get_temperature(self):
        ...
        temperature = self.__send_relay_command__(
            commands['getTemperature'], True)
        if temperature:
